...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  if (prevConfigRef.current && prevConfigRef.current[k] !== (cfg as Record<string, unknown>)[k]) {
          mustReinit = true;
          break;
        }
    }
  ...
  if (typeof window !== 'undefined' && (window as unknown as { crypto?: Crypto }).crypto?.getRandomValues) {
      ...
      const animate = () => {
        ...
        if (liquidEffect) {
          const uniformsMap = (liquidEffect as { uniforms?: Map<string, THREE.Uniform> }).uniforms;
          const uTime = uniformsMap?.get('uTime');
          if (uTime) uTime.value = uniforms.uTime.value;
        }
        if (composer) {
          if (touch) touch.update();
          composer.passes.forEach(p => {
            const effs = (p as { effects?: unknown[] }).effects;
      ...
      if (t.liquidEffect) {
  const uniformsMap = (t.liquidEffect as { uniforms?: Map<string, THREE.Uniform> }).uniforms;
